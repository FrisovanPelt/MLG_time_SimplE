...
class Tester:
    ...
    def create_queries(self, fact, head_or_tail):
        ...
        if head_or_tail == "head":
            return [(i, rel, tail, time) for i in range(self.dataset.num_ent())]
        elif head_or_tail == "tail":
            return [(head, rel, i, time) for i in range(self.dataset.num_ent())]
        ...
        for i, fact in enumerate(self.dataset.data[self.valid_or_test]):
            for head_or_tail in ["head", "tail"]:
                ...
                for raw_or_fil in settings:
                    h, r, t, time = self.add_fact_and_shred(fact, queries, raw_or_fil)
                    sim_scores = self.model(h, r, t, time).cpu().data.numpy()
