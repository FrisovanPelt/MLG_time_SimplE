...
class Dataset:
    ...
    def triple2ids(self, triple):
        return [self.get_ent_id(triple[0]), self.get_rel_id(triple[1]), self.get_ent_id(triple[2]), self.get_time_id(triple[3])]
    ...
    def get_time_id(self, time):
        if not time in self.time2id:
            self.time2id[time] = len(self.time2id)
        return self.time2id[time]
    ...
    def next_batch(self, batch_size, neg_ratio, device):
        ...
        times  = torch.tensor(batch[:,3]).long().to(device)
        labels = torch.tensor(batch[:,4]).float().to(device)
        return heads, rels, tails, times, labels
