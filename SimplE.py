...
class SimplE(nn.Module):
    def __init__(self, num_ent, num_rel, num_time, emb_dim, device):
        ...
        self.time_embs    = nn.Embedding(num_time, emb_dim).to(device)
        ...
        nn.init.uniform_(self.time_embs.weight.data, -sqrt_size, sqrt_size)
    ...
    def forward(self, heads, rels, tails, times):
        ...
        u = self.time_embs(times)
        r_embs = self.rel_embs(rels) * u
        r_inv_embs = self.rel_inv_embs(rels) * u
